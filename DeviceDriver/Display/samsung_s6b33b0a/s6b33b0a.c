...
//x for horizontal and y for vertical, so where you would expect an X address below you will find a Y Address and vice-versa.
	...
	/*
	if(wYAddressControl < 0){	//do we need to clip on top?
		wHeight += wYAddressControl;
		dataToSend = -wYAddressControl;
		wYAddressControl = 0;	
		if(wDisplayOperation & DISPLAY_COLOR){
			bitMask = 2;
		}
		else{
			bitMask = 24;		
		}
		pBuffer += dataToSend * (1+(wWidth-1)/bitMask);
	}	
	*/
	if((wXAddressControl + wWidth) > LCD_X_SIZE){	//should we clip on the right?
		...
		dataToSend = dataToSend - wWidth;		//find the delta between old and new width
	...
	/*
	if(wXAddressControl < 0){	//should we clip on the left?
		return;
	}
	*/
	//setup vertical area
			...
			for(j=0; j < wWidth; j++){	
				if(j%2 == 0){
					if(wDisplayOperation & DISPLAY_CLEAR){
						dataToSend = (g_BackgroundColor << 12) | g_BackgroundColor;	//if clearing the display then don't read from the buffer
					}
					else{
						dataToSend = *pBuffer;
					}
					HALDisplaySendPixelData(dataToSend >> 12,wDisplayOperation);
				}
				else{
					HALDisplaySendPixelData(dataToSend,wDisplayOperation);	
				}
				if(j % 2 || j == (wWidth-1)){	//may be able to optimize further with numWordsToSkip = 1 by default
					//then we need to increment the dataPtr
					pBuffer++;
				}	
			}
		...
		for(i=0; i < wHeight;i++){
			for(j=0; j < wWidth; j++){
				...
				if(bitMask & dataToSend){	//is the pixel ON?
					...
					HALDisplaySendPixelData(0x000FFF,wDisplayOperation);
				}
