...
// Initialize backlight state to true -- used when no settings are saved
INT g_iBackLightState=BACKLIGHT_ON;


/////////////////////////////////////////////////////////////////////////////////////////
//
//>  Name:          int _reentrant BacklightMenu(int a, int b, int *pPtr)
//
//   Type:          Function (code bank callable)
//
//   Description:   This function is the backlight menu
//
//   Inputs:        none used.
//
//   Outputs:       messages
    ...
    while(!bDone)
	{
		...
		switch(iEvent)
		{
			case EVENT_TIMER:
                ...
                SysCallFunction(RSRC_DISPLAY_CODE_BANK,DisplayBacklight,TRUE, 0,0);
	        ...
	        case EVENT_BUTTON:
	            ...
	            switch(g_wLastButton = gEventInfo.Button.wButtonEvent)
	            {
		            ...
		            case PH_RW:
		            case PR_RW:
						bSendToPlayerStateMachine=FALSE;	//Do not send to playerstatemachine
	        			// toggle between on/off
 			        	if (g_iBackLightState == BACKLIGHT_OFF)
						{
        			    	g_iBackLightState = BACKLIGHT_ON;
                	    	SysPostMessage(2,LCD_BACKLIGHT_ON);
	            			SysPostMessage(6,SOFT_TIMER_SET_TIMER,TIMER_BACKLIGHT,1,BACKLIGHT_TIME,MENU_MSG_TURN_OFF_BACKLIGHT);
                    	    SysWaitOnEvent(0,0,0);
						}
			        	else		//backlight is already on, turn off.
		    	    	{ 
        		    		g_iBackLightState = BACKLIGHT_OFF;
...
#pragma asm
    ...
    extern y:Fg_iBackLightState

    SaveRange Fg_iBackLightState,1
