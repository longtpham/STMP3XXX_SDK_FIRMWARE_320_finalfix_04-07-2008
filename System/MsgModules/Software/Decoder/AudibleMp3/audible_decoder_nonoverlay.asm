...
;
;///////////////////////////////////////////////////////////////////////////////
        section DECAPP_AudibleDecoderNonOverlay
...
;///////////////////////////////////////////////////////////////////////////////
       GLOBAL   AudibleDecoderModuleTimerDisable
       GLOBAL   AudibleMp3DecoderModuleIsr
       GLOBAL   AudibleDecoderFileRead
       GLOBAL   AudibleDecoderReadStatus
       GLOBAL   AudibleMp3GetInvBitRate
;///////////////////////////////////////////////////////////////////////////////
;   External References
;///////////////////////////////////////////////////////////////////////////////
       EXTERN   AudibleMp3DecoderFileHandle
       EXTERN   AudibleMp3DecProcess,Fstatus_o
       EXTERN   AudibleMp3DecIsrSp
...
;       EXTERN   FILEOpen
       ...
       EXTERN   FAudible_MP3FileRead
	   EXTERN   g_bAudiblePlayThrough
	   EXTERN   g_bAudibleConsumedBytesHigh
	   EXTERN   g_bAudibleConsumedBytesLow
	   ;EXTERN   FgMP3ReadBufSize
...
;   X Memory
;///////////////////////////////////////////////////////////////////////////////
        org     x,"DECAPP_AudibleDecoderNonOverlay_X":
AudibleDecoderReadStatus       dc  0
AudibleDecoderFileReadNBytes   dc  0
...
;   Code
;///////////////////////////////////////////////////////////////////////////////
        org     p,"DECAPP_AudibleDecoderNonOverlay_P":

;*******************************************************************************
; AudibleDecoderModuleTimerDisable Routine
;*******************************************************************************
AudibleDecoderModuleTimerDisable
...
;*******************************************************************************
; AudibleDecoderFileRead Routine
;*******************************************************************************
AudibleDecoderFileRead
        ; CANNOT be overlayed, called by ISR.
...
_return
        move    x:AudibleDecoderReadStatus,x0
...
_RestoreSystem
        ...
        move    a1,x:AudibleDecoderFileReadNBytes
...
_checkForSpace
        ...
        move    y:AudibleMp3DecoderSourceBufDesc,r4
...
_ContinueNoSync                   
	                  ...
	                  move    #>AUDIBLE_MP3_SAFE_NUMBER,B
                      cmp     x0,B
                      ...
                      add     x0,A      #>g_AudibleMp3DataSrcBufSize,x0
...
_WithinBounds
    	              move #>AUDIBLE_MP3_SAFE_NUMBER,x0    
...
; 10/05/2004 mmiu - Don't check for EOF here. Always read data and check for
; EOF later.
...
_readMoreData
		...
		; mmiu - When we get here, A1 contains the number of words we want to read
        move    y:<Const_000003,x0
        move    a,x1
        mpy     x0,x1,a                     ; get number of bytes (from words)
        asr     a                           ; fix for integer arithmetic
        move    a0,a                        ; A1 = num bytes to read
;        move    y:AudibleMp3DecoderFileHandle,a2   ; A2 = File handle
;        move    #TARGET_MEM_X,a0            ; A0 = Memory space identifier
        move    y:g_pAudibleMp3DataSrcBufHead,r0   ; R0 = Buffer pointer to read into
        ...
        move    y:g_pAudibleMp3DataSrcBufMod,m0    ; M0 = Buffer modulo
        
;        jsr     FileRead                    ; FileRead A1 num bytes into r0 buf

        ; mmiu - Redirect FileRead to Audible_MP3FileRead. Returns bytes read in A1
		; int Audible_MP3FileRead( _circ int _X * pDest, int numBytes )
		;                          R0,M0                 A1
		jsr     FAudible_MP3FileRead
		tst		A							; Check for error
        jgt     _swapMsbLsb					; If no error, swap MsbLsb
        pop     x1                          ; Otherwise, error or EOF
        pop     r0                          ; For both cases, balance the stack
        clr     a							; Return 0 bytes read
        ; Must set EndOfFileReached instead of EndOfSong as in previous versions. STMP1851.
        move    x:DecoderCSR2,x0
        bset    #EndOfFileReached,x0        
        move    x0,x:DecoderCSR2
;        move    #1,x0                       ; Set played through indicator flag
;        move    x0,x:g_bAudiblePlayThrough
		jmp     _return
...
_swap_msb_lsb
        ; update the head ptr
        move    r0,y:g_pAudibleMp3DataSrcBufHead    ; adjust the pointer
        move    y:<Const_ffffff,m0
...
GetMpegSrcBufWords
        ...
        clr     b        
        move    #>g_AudibleMp3DataSrcBufSize,b
...
_DecoderSaveRegReturn
        ...
        move    #AudibleMp3DecIsrSp,r5                 ; pointer to the SP local save
        jmp     SaveSystemStack              ; save the hardware stack
...
_checkForData
        move    y:AudibleMp3DecoderSourceBufDesc,r4 ; TIME_FIX
        move    y:<Const_ffffff,m4
        jsr     GetEmptySpace
...
_getMoreBytes
        ...
        mpy     y0,x0,a
        asr     a        
        ; We tally consumed bytes when buf free bytes decreases across an AudibleMp3DecProcess.
...
_ForceInt
        ...
        clr     a
        move    x:AudibleDecoderFileReadNBytes,a1      ; TIME_FIX note _incrementByteCount
...
_ok
        ...
        jsr     AudibleDecoderModuleTimerDisable    ; turn off timer
...
_restore_stack
        ...
        jsr     AudibleMp3GetInvBitRate             ; used for timer count and time. must be called each time in case VBR.
        move    x:DecoderCSR,y0              ; copy contents of status register
        move    #>DecoderTimerNum,x0         ; input to timer routine
        move    y:g_AudibleMp3TimerCount,x1         ; mp3dec.asm fills this in
_progtimer
...
;        trace   #>$AAFFFF
        ...
        btst    #DECODER_READ_ENABLE_BIT,x:AudibleDecoderReadStatus
        jcc     _End
        
        ; Clears the enable bit
        bchg    #DECODER_READ_ENABLE_BIT,x:AudibleDecoderReadStatus
        ...
        nop
        movec   #AudibleDecoderFileRead,ssh   ;Forces AudibleDecoderFileRead call after rti
        nop
_End
        rti                            ;Int level is dropped to 0 (user level)
                                       ;AudibleDecoderFileRead() operates at user level


;*******************************************************************************
; Name:        AudibleMp3GetInvBitRate
; Type:        SubRoutine
;
