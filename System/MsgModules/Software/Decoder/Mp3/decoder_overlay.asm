...
;///////////////////////////////////////////////////////////////////////////////
       GLOBAL  Mp3DecoderModule
       GLOBAL  Mp3DecoderModuleInit
...
;///////////////////////////////////////////////////////////////////////////////
        ...
        EXTERN   Mp3DecReset,Mp3DecInit,Mp3DecProcess
        EXTERN   MpegDecFree_SyncThresholdBase,MpegDecSet_SyncThresholdBase
        EXTERN   Fg_CurrentSongName,Mp3DecoderFileHandle,Fg_CurrentSongFastkey
        EXTERN   CurrentDirDevId
        EXTERN   DecoderModuleTimerDisable
        EXTERN   Mp3DecoderModuleIsr
        EXTERN   SysEnableTimer
        EXTERN   TotalSongCount
        EXTERN   DecoderReadStatus
        EXTERN   Fbitrate_o
...
;	EXTERN    MEM_P_ORG_SYS
    ...
    EXTERN    Mp3GetInvBitRate
    EXTERN    FMp3DecoderStartFrameFilePosition
...
;   Code
;///////////////////////////////////////////////////////////////////////////////
       org      p,"DECMOD_DecoderOverlay_overlay_P":

        ; Resource number
        dc      RSRC_DECMOD_CODE

;*******************************************************************************
; Name:        Mp3DecoderModuleInit
...
;        jsr     SysFlushCoderMemory
        ...
        move    #Mp3DecoderModule,x0
        move    x0,y:DecoderProcessPtr

    if (@def('DEVICE_3400')||@def('DEVICE_3300'))
        ; Load the MP3 decoder into P memory
        move    #>RSRC_MP3P_CODE,x0             ; Resource number
        ...
        move    #>RSRC_MP3X_CODE,x0             ; Resource number
        move    #>RSRC_TYPE_DATA,x1             ; Resource type is data
        ...
        move    #>RSRC_MP3Y_CODE,x0             ; Resource number
        move    #>RSRC_TYPE_DATA,x1             ; Resource type is data
	...
	move    #>F_lc_u_b_MP3_Y_buf,y0      ;  Address of the overlay area
        ...
        asr     A
        asr     A       #0,B
        asr     A       y:Mp3DecoderFfwdRwndValue,B0
        cmp     b,a
        tgt     b,a
        ...
        do      #g_Mp3DecSinkBufSize,_clear_pcmbuffer
         move    a,y:(r3)+
         nop
_clear_pcmbuffer

        move    #>g_Mp3DataSrc_mbuf,r3
        do      #g_Mp3DataSrcBufSize,_clear_bitbuffer
         move    a,x:(r3)+
         nop
_clear_bitbuffer

        ; Initialize for Mp3 Decoder
        move    #g_Mp3DataSrcBufDescriptor,x0
        move    x0,y:Mp3DecoderSourceBufDesc  ; decoder source buffer descriptor
        move    #g_Mp3DecSinkBufDescriptor,x0
        move    x0,y:Mp3DecoderSinkBufDesc    ; decoder sink buffer descriptor
		...
		move	#>g_Mp3DecSinkBufDescriptor,A
		move	#>TARGET_MEM_Y,B
...
_InitEventValue
        ...
        move    #>500,x0
        jsr     ModuleSetTimer
        move    a1,y:Mp3DecoderEventValue
...
_AdvanceToNextSong
        ...
        move    #>SPEED_CLIENT_MP3_DEC,y0
        jsr     SysSpeedClockFree             ;Free DCLK for other speed clients
        move    #>SPEED_CLIENT_MP3_DEC_SYNC,y0
        jsr     SysSpeedClockFree             ;Free DCLK for other speed clients
        ...
        move    x0,y:Mp3DecoderEventValue
...
_stackRestored
        ...
        move     y:Mp3DecoderEventValue,a1
...
_handleCheckABBoundariesError
        ...
        move    #>SPEED_CLIENT_MP3_DEC,y0       ; speed client ID
...
; this routine sets Mp3DecoderEventValue up.
        move    #>EVENT_MESSAGE,a1
        move    #>500,x0
        jsr     ModuleSetTimer
        move    a1,y:Mp3DecoderEventValue
...
; debug   ;//dbgmpg
        ...
        jset    #EndOfFileReached,x0,_NextMessage
        move    x:DecoderCSR,x0
        jset    #EndOfSong,x0,_NextMessage
...
_beginPlay
        ...
        move    r7,x:Mp3DecoderSaveR7   ; temp -- later need to strip r7 out of the routines
        move    m7,x:Mp3DecoderSaveM7   ; temp -- later need to strip m7 out of the routines

        ; Increase speed to SPEED_MAX. Allows faster sync times.
        ; After syncing, speed is dropped to SPEED_MP3 for bitrates < 256 kbps.
        move    #>SPEED_MAX,x1
        move    #>SPEED_CLIENT_MP3_DEC_SYNC,y0  ; Speed client identifier
        ...
        move    x:Mp3DecoderSaveM7,m7 ; Restore r7,m7 on DecoderOpenSong error.
        move    x:Mp3DecoderSaveR7,r7
...
_UpdateTime
        ...
        move    x:Mp3DecoderSaveM7,m7
        ; Assertion testpoint for r7 and Mp3DecoderSaveR7 in valid range
        move    x:Mp3DecoderSaveR7,r7
...
_WaitForSync
        ...
        move    #>EVENT_NONE,x0
        move    x0,y:Mp3DecoderEventValue  ; need to be called every time
...
_FadeIn
        ...
        ; ** DROP SPEED FROM SPEED_MAX to SPEED_MP3 ONLY IF BITRATE < 256 kbps
        ; THIS IS TO ALLOW HIGH BITRATE SONGS TO NOT HAVE THE SLOW FADE IN PROBLEM
        ; (due to Mixer Fade In getting blocked in the user level)
        ; THIS MUST BE DONE PRIOR TO FADEIN CALL BECAUSE SysSpeedIncrease switches
        ; TO CRYSTAL (24.576 MHz) and decoder immediately falls behind causing
        ; DAC underflows and glitches in the sound.
        move    x:Fbitrate_o,a
        move    #>256000,x0
        cmp     x0,a
        jge     _ExitClockChange
	if (@def('SYNC_LYRICS'))
        ; Check if VBR, don't drop the clock speed for lyrics display
        move	x:Fg_bClockSpeedUp,a
        move    y:<Const_000000,x0
        cmp		x0,a
        jeq		_ClockChange				; if Fg_bClockSpeedUp has not been set, drop the speed to normal
        move    y:VBRSignature,a
        move    y:<Const_000000,x0          ; See if non-VBR
        cmp     x0,a
        jne		_ExitClockChange
_ClockChange
	endif
        ; Request SPEED_MP3 with other client.
        ; SPEED_CLIENT_DEC_SYNC free call right after this restores DCLK to SPEED_MP3.
        move    #>SPEED_MP3,x1
        move    #>SPEED_CLIENT_MP3_DEC,y0   ; Speed client identifier
        jsr     SysSpeedIncrease            ; Manages clock speed clients
        ; Free DCLK for other speed clients
        move    #>SPEED_CLIENT_MP3_DEC_SYNC,y0
        jsr     SysSpeedClockFree
...
_FileOpenedForFfwd
        ...
        ;move    y:Mp3DecoderFfwdRwndValue,x1
        cmp     A,B                    ; if num bytes to skip < num bytes left, ffwd
        jgt     _bytesLeftPassed
...
_bytesLeftPassed
	...
	; EffectsInit should be called before calling any DAC initialization to ensure the processing chain is loaded.
    jsr	EffectsInit
        jsr     Mp3DecInit              ; init the buffers, reset head/tail ptrs, reset MP3 decoder
        jsr     DacDriverInit

        jsr     MpegDecFree_SyncThresholdBase ;prevent sync threshold failure on FF
        ; FileSeek forward
        clr     a
        move    y:Mp3DecoderFfwdRwndValue,a0
        clr     b
        move    y:Mp3DecoderFfwdRwndValue,b0
        jsr     DecoderSeek ; FSeeks & Increments SongByteTotal & SongByteTotalConsumed if ok
        jsr     MpegDecSet_SyncThresholdBase ;reset base for sync attempt count
        jsr     Mp3GetInvBitRate            ; needed for songtime calculation
        jsr     DecoderUpdateCurrentTime    ; 0:00 / ----
...
_goToPrevSong
        ...
        ; When moving to prev song via _Rwnd, Free DCLK for other speed clients
        move    #>SPEED_CLIENT_MP3_DEC,y0
        jsr     SysSpeedClockFree
        ; Free DCLK for other speed clients
        move    #>SPEED_CLIENT_MP3_DEC_SYNC,y0
...
_FileOpenedForRwnd
		...
		move	y:<Const_000001,x0          ; Always Stereo output
		jsr		EffectsInit
        jsr     Mp3DecInit ;init buffers, reset head/tail ptrs, reset MP3 decoder
        ...
        move    y:Mp3DecoderFfwdRwndValue,x0
        move    y:Mp3DecoderFfwdRwndValue,y0
...
_ConsumedFloorDone
        ...
        jsr     MpegDecFree_SyncThresholdBase ; Free it: sync attempts postponed
...
_checkA
        ...
        move    a1,y:Mp3ABPositionAHigh ; save consumed high byte //FOR LOOPING BACK TO "A", SEEK TO CONSUMED BYTES AS OPPOSED TO READ BYTES, SO THAT YOU DON'T LOSE DATA
        move    a0,y:Mp3ABPositionALow  ; save consumed low byte
        move    a1,y:Mp3ABPositionAConsumedHigh ; ditto
        move    a0,y:Mp3ABPositionAConsumedLow
...
_checkB
        ...
        move    a1,y:Mp3ABPositionBHigh    ; save high byte. consumed version
        move    a0,y:Mp3ABPositionBLow     ; save low byte

        ; make sure AconsumedPos < Bpos  (both are consumption positions)
        clr     b
;        move    y:Mp3ABPositionAHigh,b1    ; B1 = PosAHigh
;        move    y:Mp3ABPositionALow,b0     ; B0 = PosALow
        move    y:Mp3ABPositionAConsumedHigh,b1    ; B1 = PosAHigh consumed ver
        move    y:Mp3ABPositionAConsumedLow,b0     ; B0 = PosALow  consumed ver
...
InitInputBuffer
        ...
        move    #g_Mp3DataSrcBufDescriptor,r0     ; buffer descriptor ptr
        move    y:<Const_ffffff,m0
        jsr     InitBufferEmpty
_exit
        rts
...
;        move    #>READMODE,a0                ; A0 = file open attribute (Read Only)
  ...
  if (@DEF('USE_PLAYLIST5'))
	...
	move    y:(r0),A1
        ...
        jcs     _exit
        move    a1,y:Mp3DecoderFileHandle       ; store the handle for the file
        move    x:DecoderCSR,x0
        bset    #FileIsOpen,x0
...
;-----------------------------------------------------------------
; DecoderCloseSong
; This function will use the Mp3DecoderFileHandle to close the file.
;-----------------------------------------------------------------
DecoderCloseSong
        move    x:DecoderCSR,x0
        ...
        jsr     SysAllLinear
        clr     a
        move    y:Mp3DecoderFileHandle,a2       ; pass the file handle to the function
        jsr     FILEClose
...
; Sends message to MENU to turn off AB icons.
        ...
        move    x0,y:Mp3ABPositionAHigh
        move    x0,y:Mp3ABPositionALow
        move    x0,y:Mp3ABPositionAConsumedHigh
        move    x0,y:Mp3ABPositionAConsumedLow
        move    x0,y:Mp3ABPositionBHigh
        move    x0,y:Mp3ABPositionBLow
...
DecGetABModeBPos
; Returns: B1 = Mp3ABPositionBHigh , B0 = Mp3ABPositionBLow
        clr     b
        move    y:Mp3ABPositionBHigh,b1
        move    y:Mp3ABPositionBLow,b0
        rts
...
;    testpoint
        ...
        clr     a
        move    y:Mp3ABPositionAHigh,a1
        move    y:Mp3ABPositionALow,a0
        move    a1,y:SongByteTotalHigh
        move    a0,y:SongByteTotalLow
        ; TIME_FIX  Also update bytes consumed
        clr     b
        move    y:Mp3ABPositionAConsumedHigh,b1
        move    y:Mp3ABPositionAConsumedLow,b0
