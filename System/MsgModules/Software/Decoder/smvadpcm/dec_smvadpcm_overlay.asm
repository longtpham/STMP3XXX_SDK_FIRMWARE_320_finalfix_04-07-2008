...
;///////////////////////////////////////////////////////////////////////////////
        ...
        section DECMOD_AdpcmSmv


;///////////////////////////////////////////////////////////////////////////////
;   External Definitions
;///////////////////////////////////////////////////////////////////////////////
       GLOBAL  SmvAdpcmDecoderModule
       GLOBAL  SmvAdpcmDecoderModuleInit

;///////////////////////////////////////////////////////////////////////////////
;   External References
;///////////////////////////////////////////////////////////////////////////////
       EXTERN	DecAdpcmSmvModuleTimerDisable
       EXTERN	DecAdpcmSmvModuleIsr
       EXTERN	DecoderCSR
       EXTERN	DecoderSR
       EXTERN	DacDriverIsr
       ...
       EXTERN 	F_lc_u_b_SMVADPCM_Dec_P
       EXTERN 	F_lc_u_e_SMVADPCM_Dec_P
       EXTERN 	F_lc_u_b_SMVADPCM_Dec_X
       EXTERN 	F_lc_u_e_SMVADPCM_Dec_X
       EXTERN   F_lc_u_b_Jpeg_P
       EXTERN   F_lc_u_e_Jpeg_P
       EXTERN   F_lc_u_b_Jpeg_X
       EXTERN   F_lc_u_e_Jpeg_X
       EXTERN   F_lc_u_b_SmvJpeg_Y
       EXTERN   F_lc_u_e_SmvJpeg_Y
       EXTERN	SysSetupDecVectors
       EXTERN	TotalSongCount
       EXTERN	SmvAdpcmDecoderSinkBufDesc
       EXTERN	g_SmvAdpcmDecSinkBufDescriptor
       EXTERN	DecAdpcmSmvProcess
       ...
       EXTERN   g_pSmvAdpcmDecSinkBufSize
       EXTERN   FillBufferY
       EXTERN   MsgGeqSetCoef
       EXTERN   MsgGeqParam
       EXTERN   Fg_CurrentSongName
       EXTERN   smv_wave_header_size
        ; for parser
       ...
       EXTERN   SmvAdpcmABPositionAHigh
       EXTERN   SmvAdpcmABPositionALow
       EXTERN   SmvAdpcmABPositionBHigh
       EXTERN   SmvAdpcmABPositionBLow 
       EXTERN   PlayMessage
       ...
       extern Decode_SMV_ADPCM_Block
       ;extern Decode_IMA_ADPCM_Block
       ;extern Decode_PCM_Block
       
       EXTERN   FileSeek
       EXTERN   smvwave_file_offset
       EXTERN	EffectsInit
...
;        nolist
        ...
        include "dec_smvadpcm_overlay.xref"
...
;   Code
;///////////////////////////////////////////////////////////////////////////////
        org     p,"DECMOD_AdpcmSmv_overlay_P":

        ; Resource number
        dc      RSRC_DEC_SMVADPCM_MOD_CODE

;*******************************************************************************
; SmvAdpcmDecoderModuleInit
...
_stackSaved
        ...
        move    #SmvAdpcmDecoderModule,x0
        move    x0,y:DecoderProcessPtr

        ; RSRC_DEC_SMVADPCMP_CODE would include the SmvJpeg P code
        ...
        move    #>RSRC_DEC_SMVADPCMX_CODE,x0       ; Resource number
        move    #>RSRC_TYPE_DATA,x1             ; Resource type is data
        move    #>(F_lc_u_e_SMVADPCM_Dec_X-F_lc_u_b_SMVADPCM_Dec_X)*3,y1 ; Size of the overlay area
        move    #>F_lc_u_b_SMVADPCM_Dec_X,y0                       ; Address of the overlay area
        move    #TARGET_MEM_X,a1                ; Overlay area is in X memory
        jsr     SysLoadResource

        ; Load the Jpeg decoder into X memory
        move    #>RSRC_SMVJPEG_DECODER_X,x0             ; Resource number
        move    #>RSRC_TYPE_DATA,x1             ; Resource type is data
        move    #>(F_lc_u_e_SmvJpeg_X-F_lc_u_b_Jpeg_X)*3,y1 ; overlay area sz
        ...
        move    #g_SmvAdpcmDecSinkBufDescriptor,x0
        move    x0,y:SmvAdpcmDecoderSinkBufDesc      ; decoder sink buffer descriptor
        ...
        move    y:Fg_SmvWave_wChannels,A
        asr     A   #>TARGET_MEM_Y,B
        move    A,x0
	    move	#>g_SmvAdpcmDecSinkBufDescriptor,A
	    jsr	    EffectsInit

        ; Init the interrupt vectors
        move    #>DacDriverIsr,x0
        move    #>DacDriverIsr,x1
        move    #>DecAdpcmSmvModuleIsr,y0
...
_checkForSync
        ...
        move    y:Fg_SmvWave_dwSamplesPerSec,x0
...
_CheckIfEndOfList
        move    x:DecoderCSR,x0
        jset    #EndOfList,x0,_exitSmvAdpcmDecoderModule
_CheckIfEndOfSong
         move   x:DecoderCSR,x0
         jclr   #EndOfSong,x0,_exitSmvAdpcmDecoderModule
...
_FileReadError
        ...
        move    #>EVENT_MESSAGE,x0
        move    x0,y:AdpcmDecoderEventValue
        jmp     _exitSmvAdpcmDecoderModule
...
_ContinueProcessing
        ...
        ;move    y:TotalSongCount,a     ; In the present implementation, if TotalSongCount is 0, you could not stop the decoder,
        move    #>1,a
...
;        jsr     SmvAdpcmInitBuffers             ; init the buffers, reset head/tail ptrs
_checkPause
        move    x:DecoderCSR,x0
...
_Fast_forward
        ...
        move    y:Fg_SmvWave_dwAvgBytesPerSec,y0
        mpy     x0,y0,A    
        asr	A  
...
_Rwnd
        ...
        jsr     SysPostMessage
_FileOpenedForRwnd
		move	y:SmvRewindMode,A				 ; Other decoders ignore this.
        tst     A       #0,x0
        jeq     _ContinueRwnd
...
_checkA
        ...
        move    a1,y:SmvAdpcmABPositionAHigh    ; save high byte
        move    a0,y:SmvAdpcmABPositionALow     ; save low byte
        jmp     _NextMessage
_checkB
        ...
        jsr     DecGetSongPos           ; returns byte position in A1:A0
;        jsr     DecSetPosBoundary       ; sets the position to the adpcm boundary
        move    a1,y:SmvAdpcmABPositionBHigh    ; save high byte
        move    a0,y:SmvAdpcmABPositionBLow     ; save low byte

        ; make sure A < B
        clr     b
        move    y:SmvAdpcmABPositionAHigh,b1    ; B1 = AHigh
        move    y:SmvAdpcmABPositionALow,b0     ; B0 = ALow
...
_CheckDone
        ...
        nop 
        jmp     (r6)                      ; return to SmvAdpcmDecoderModule routine
...
;*******************************************************************************
DecSmvAdpcmInit
        move    y:SmvAdpcmInitFlag,A
        tst     A   #0,A
        jne     _Exit
        move    A,y:SmvAdpcmInitFlag

	jsr     SysAllLinear
        ; Make sure we start at the right place
        clr     a                            ; offset of 0
        move    A,y:smvwave_file_offset
...
_Continue
		...
		move	y:Fg_SmvWave_datachunksize,A
...
DecoderSeek
        ...
        move    y:(Fg_SmvWave_data_offset+1),b
        move    y:Fg_SmvWave_data_offset,b0
        sub     b,a        #0,x0
        tlt     x0,a                     ; We have reached the beginning of the file.
        jlt     _Continue


        move    y:Fg_SmvWave_wBlockAlign,y0
...
_Continue
	...
	move	y:SongByteLengthLow,B0
       	sub     A,B
	move	B0,y:Fg_SmvWave_datachunksize
        move	B1,y:(Fg_SmvWave_datachunksize+1)
...
_notYetOpen
    ...
    if (@DEF('USE_PLAYLIST5'))
	    ...
	    move    y:(r0)+,A0
	    move    y:(r0),A1
        move    #>READMODE,b                ; b = file open attribute (Read Only)
...
; Sends message to MENU to turn off AB icons.
        ...
        move    x0,y:SmvAdpcmABPositionAHigh
        move    x0,y:SmvAdpcmABPositionALow
        move    x0,y:SmvAdpcmABPositionBHigh
        move    x0,y:SmvAdpcmABPositionBLow
...
DecGetABModeBPos
; Returns: B1 = SmvAdpcmABPositionBHigh , B0 = SmvAdpcmABPositionBLow
        clr     b
        move    y:SmvAdpcmABPositionBHigh,b1
        move    y:SmvAdpcmABPositionBLow,b0
        rts
...
; Sets the song position to AB position "A"
        clr     a
        move    y:SmvAdpcmABPositionAHigh,a1
        move    y:SmvAdpcmABPositionALow,a0
        tst     a
...
; Output: A1:A0 has a0 fixed on the song boundary
        ...
        move    #>$fffe00,x0                 ; assumes Smv_Dec_nblockalign = 512 ($200)
        clr     b
        move    a0,b1                        ; b1 = low value of song bytes
        and     x0,b                         ; line up on a block boundary
        move    y:smv_wave_header_size,x0
...
DecoderClearSinkBuffer
        ...
        move    #g_SmvAdpcmDecSinkBufDescriptor,r0      ; SINK buffer descriptor ptr
        move    #>$ffff,m0
        jsr     InitBufferEmpty
        ; clear the sink buffer for adpcm decode routines
        clr     a
        move    y:<Const_ffffff,m3
        move    #>g_SmvAdpcmDecSink_mbuf,r3
        move    #>g_SmvAdpcmDecSinkBufSize/2,x0
...
Init_ADPCM_States
        ...
        jsr     SysAllLinear
        clr     A
        move    A,y:Smv_Last_block_flag
        ; By default decode Samples per block
        move    y:Fg_SmvWave_wnSamplesPerBlock,A
        move    A,y:SmvSamplestoDecode
        ; By default input buffer should be block align in size.
        move    y:Fg_SmvWave_wBlockAlign,A
        move    A,y:smvinputbufsize 	
        jsr     FSmv_Init_IMA_ADPCM_States
...
GetInvBitRate
    move    y:Fg_SmvWave_dwAvgBytesPerSec,B
...
SetDecoderType
		...
		;cmp		x0,B	#Decode_SMV_ADPCM_Block,r6
		;jeq		_EndDecodeBlock
        ;endif
...
_EndDecodeBlock
		clr		A    r6,y:SmvAdpcmDecoderPointer
		rts
        endsec
