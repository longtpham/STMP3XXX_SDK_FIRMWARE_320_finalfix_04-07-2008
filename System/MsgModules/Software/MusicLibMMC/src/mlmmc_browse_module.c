...
==================================================================================================*/
COMP_OPT uint24 ML_BrowseMenu( uint24 item_type, uint24 action, uint24 highlight, uint24 hasSD, FLASH_GROUP_NAME_T *item_list, uint24 *num_item, uint24 *total_num_item, uint24 *isEndList );		// fail return 0; success return 1
COMP_OPT uint24 ML_DownList( uint24 step, FLASH_GROUP_NAME_T *item_list, uint24 *num_item, uint24 *isEndList ); // fail return 0; success return 1
COMP_OPT uint24 ML_UpList( uint24 step, FLASH_GROUP_NAME_T *item_list, uint24 *num_item, uint24 *isEndList );
...
COMP_OPT uint24 ML_BrowseMenu( uint24 item_type, uint24 action, uint24 highlight, uint24 hasSD, FLASH_GROUP_NAME_T *item_list, uint24 *num_item, uint24 *total_num_item, uint24 *isEndList ) {
	...
	*total_num_item = 0;
	...
	/* Get list from SD memory */
	if ( hasSD )
		ML_GetListAndID( 0, current_state, MEDIA_TYPE_SD, 0, &sd_name_list_size, &i );
	else
		sd_name_list_size = 0;
...
COMP_OPT uint24 ML_DownList( uint24 step, FLASH_GROUP_NAME_T *item_list, uint24 *num_item, uint24 *isEndList ) {
...
COMP_OPT uint24	ML_SelectItemNameByPosition( uint24 current_state, uint24 flash_pos, uint24 sd_pos, uint24 *out_flash_pos, uint24 *out_sd_pos, uint24 *id1, uint24 *id2, uint24 *nameIndex ) {
	...
	if ( (name[0].name[0]|name[1].name[0]) == 0 ) {
		...
		*nameIndex = 0xFFF;
        ...
        while (g_SortByTracknum && ((current_state==STATE_ARTIST_LVL_3) || (current_state==STATE_ALBUM_LVL_2))) {
            ML_GetTracknums( 0, current_state, flash_pos, sd_pos, &tracknum1, &tracknum2, id1, id2);

            if ((tracknum1>tracknum2) || (*id1 == 0xFFF)) {
            	    sd_pos++;
            	    *id1 = 0xFFF;
        	        *nameIndex = 1;
	        } else if ((tracknum2>tracknum1) || (*id2 == 0xFFF)) {
            	    flash_pos++;
            	    *id2 = 0xFFF;
        	        *nameIndex = 0;
	        } else
...
COMP_OPT void ML_GetTotalSongs( uint24 *flash_song, uint24 *sd_song, uint24 hasSD ) {
	uint24 id;

	if ( hasSD )
		ML_GetListAndID( 0, STATE_TRACK_LVL_1, MEDIA_TYPE_SD, 0, sd_song, &id );
	else
		*sd_song = 0;

	ML_GetListAndID( 0, STATE_TRACK_LVL_1, MEDIA_TYPE_FLASH, 0, flash_song, &id );
}
...
COMP_OPT void	ML_SearchLastWindow( FLASH_GROUP_NAME_T *item_list, uint24 *num_item ) {
	...
	for ( i=0;i<g_iBrowseWindowSize-1;i++ ) {
		ML_UpList( 1, item_list, num_item, &temp );
	}
...
/* 050420 Add for ON The Go */
COMP_OPT uint24 LOCAL_AddOnTheGoList(uint24 op) {
uint24 i;
uint24 j;
uint24 id;
uint24 found;

	if (op == ON_THE_GO_DEL_ALL) {
		on_the_fly_list.num_of_id = 0x0;
		if (IsPlayOnTheGo) {
			play_flash_name_list_size = 0x0;
			play_total_merge_num_item = 0x0;
		}
		return ON_THE_GO_FREE;
	}
	id = OnTheGoPlayID;
	if (OnTheGoPlayMedia == MEDIA_TYPE_SD) {
		id += MAX_NUM_OF_SONG;
	}
	found = FALSE;
	for (i=0; i<on_the_fly_list.num_of_id;i++) {
		if (on_the_fly_list.store_list[i] == OnTheGoThisID) {
			if (op == ON_THE_GO_TEST) {
				return ON_THE_GO_EXIST;
			} else if (op == ON_THE_GO_DEL) {
				for (j=i;j<(on_the_fly_list.num_of_id-1);j++) {
					on_the_fly_list.store_list[j] = on_the_fly_list.store_list[j+1];
				}
				on_the_fly_list.num_of_id--;
				if (IsPlayOnTheGo) {
					play_flash_name_list_size--;
					play_total_merge_num_item--;
					if (id == OnTheGoThisID) {
						return ON_THE_GO_DEL_PLAYING;
					} else if (!found) {
						return ON_THE_GO_DEL_SMALLER_ID;
					}
				}
				return ON_THE_GO_FREE;
			}
		}
		if ((!found) && (on_the_fly_list.store_list[i] == id)) {
			found = TRUE;
		}
	}
	if (on_the_fly_list.num_of_id == NUM_OF_SONG_IN_ON_THE_FLY) {
		return ON_THE_GO_FULL;
	}
	if (op != ON_THE_GO_ADD) return ON_THE_GO_FREE;

	on_the_fly_list.store_list[on_the_fly_list.num_of_id++] = OnTheGoThisID;
	if (IsPlayOnTheGo) {
		play_flash_name_list_size++;
		play_total_merge_num_item++;
	}
...
COMP_OPT void LOCAL_GetID( uint24 highlight) {
	//uint24 temp;
	uint24 id, id2;

	if (highlight < g_iBrowseWindowSize) {
		// store current window position
		ML_GetIDFromCurrentWindow( highlight, &id, &id2 );
		OnTheGoThisID = (id & 0xFFF);
		if (OnTheGoThisID == 0xFFF) {
			OnTheGoThisID = (id2 & 0xFFF) + MAX_NUM_OF_SONG;
		}
	} else {
		OnTheGoThisID = lastSongID;
		if ((playMode & 8) == 8) {
			OnTheGoThisID += MAX_NUM_OF_SONG;
		}
	}
}
/* 050420 Add for ON The Go */
...
#ifdef _HARDCODE_TEST_
        ML_ConstructSongList();
#endif
...
int _reentrant MusicLibBrowseModuleProcessQueue(union WaitMask Signals,struct MessageQueueDescriptor * pQueue)
{
	...
	if(Signals.B.m_bMessage) {
		while(GetMessage(pQueue,&Msg)) {
            ...
            switch(Msg.m_uMsg[0])
            {
            	case MUSICLIB_BROWSE_SETROOT:
            		ML_BrowseMenu( ITEM_TRACK, ACTION_MENU, 0, g_is_SD_inserted, browse_item_name, &num_item, (uint24*)Parm1, (uint24*)&temp );
            		break;
            	case MUSICLIB_BROWSE_UPMENU:
				ML_BrowseMenu( Parm2, ACTION_BACK, Parm3, g_is_SD_inserted, browse_item_name, &num_item, &total_item, (uint24*)&temp );
				*((uint24*)Parm1) = (uint24)num_item;
				break;
			case MUSICLIB_BROWSE_BROWSEMENU:
				ML_BrowseMenu( Parm2, ACTION_OK, Parm3, g_is_SD_inserted, browse_item_name, &num_item, &total_item, (uint24*)Parm4/*bHaveNext*/ );
				*((uint24*)Parm1) = (uint24)num_item;
				break;
			case MUSICLIB_BROWSE_UPLIST:
				ML_UpList( Parm1, browse_item_name, (uint24*)Parm3, (uint24*)Parm2 );
				break;
			case MUSICLIB_BROWSE_DOWNLIST:
				ML_DownList( Parm1, browse_item_name, &num_item, (uint24*)Parm2/*bHaveNext*/);
				break;
		case MUSICLIB_BROWSE_SONGNUMBER:
			ML_GetTotalSongs( (uint24*)Parm1, (uint24*)Parm2, (uint24)Parm3 );
			break;
		case MUSICLIB_BROWSE_ON_THE_FLY_GET_STATUS:
			LOCAL_GetID(Parm1);
			*((uint24*)Parm2) = (uint24)LOCAL_AddOnTheGoList(ON_THE_GO_TEST);
			break;
		case MUSICLIB_BROWSE_ON_THE_FLY_ADD_ITEM:
			LOCAL_AddOnTheGoList(ON_THE_GO_ADD);
			break;
		case MUSICLIB_BROWSE_ON_THE_FLY_DELETE_ITEM:
			LOCAL_GetID(Parm1);
			*((uint24*)Parm2) = (uint24)LOCAL_AddOnTheGoList(ON_THE_GO_DEL);
			break;
		case MUSICLIB_BROWSE_ON_THE_FLY_DELETE_ALL:
			LOCAL_AddOnTheGoList(ON_THE_GO_DEL_ALL);
			break;
		default:
			break;
			}
